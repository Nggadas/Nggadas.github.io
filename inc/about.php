<h1>About Me</h1>
<img class="profile_pic" src="../assets/images/profile_pic.jpg" alt="profile picture">
<p>I am a recent graduate of Leeds Beckett university where I studied and got a bachelors degree in Computing.
    During the course of my time there, I gained some knowledge and skills in web development and desktop
    app development as well as networking, systems modelling and computer security. I pride myself on my willingness
    to do my best possible on every project I take.</p>

<!--Skills-->
<div class="skills">
    <h3>Programming languages I work with include: </h3>

    <ul>
        <li>Front-end Development
            <ul>
                <li>HTML</li>
                <li>CSS</li>
                <li>JavaScript (jQuery)</li>
            </ul>
        </li>
    </ul>

    <ul>
        <li>Back-end Development
            <ul>
                <li>PHP (Laravel)</li>
                <li>MySQL</li>
                <li>Ruby (Ruby on Rails)</li>
            </ul>
        </li>
    </ul>

    <ul>
        <li>Desktop-app/Console-app Development
            <ul>
                <li>Java</li>
                <li>C&#35;</li>
                <li>Python</li>
            </ul>
        </li>
    </ul>
</div>